...
/*   Updated: 2021/10/18 15:18:53 by yarroubi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_runchild(t_data *data, int idx)
{
	...
	ret = pthread_create(&(data->philosophers[idx].tid), 0, ft_simulate, \
...
int	ft_launch_simulation(t_data *data)
{
	...
	ft_usleep(CREATE_TIME);
	i = -1;
	while (++i < data->attr->nb_philosophers)
		sem_post(data->launch_lock.key);
	return (0);
}
